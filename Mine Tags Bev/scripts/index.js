...
var GroupDropItemType;
(function (GroupDropItemType) {
    GroupDropItemType["Random"] = "Random";
    GroupDropItemType["All"] = "All";
})(GroupDropItemType || (GroupDropItemType = {}));
class DroppingItem {
    static log(planksId) {
        const data = {
            tag: "Woodcutter",
    ...
    static coalOre(cobbleStone) {
        const chance = Math.round(Math.random() * 5) + 10;
        const item = new ItemStack("minecraft:charcoal");
...
system.run(() => {
    ...
    class ChangeDrop {
        ...
        updateMultiplier() {
            if (!this.tool)
                return;
            const enchantable = this.tool.getComponent("minecraft:enchantable");
            if (!enchantable)
                return;
        ...
        getFortuneMultiplier(fortuneLevel) {
            let multiplier;
        ...
        init() {
            ...
            if (data.tool && !this.tool)
                return;
            if (!this.isCorrectTool(data.tool))
                return;
            const hasTag = this.player.hasTag(data.tag) && data.have !== undefined;
            const dontHasTag = !this.player.hasTag(data.tag) && data.dont !== undefined;
            if (hasTag)
                this.spawnItems(data.have);
            else if (dontHasTag)
                this.spawnItems(data.dont);
        }
        getKey() {
            ...
            if (data.type === GroupDropItemType.Random) {
                ...
                system.run(() => {
                    for (const drop of data.items) {
                        if (drop)
                            this.spawnEntityOrItem(drop);
                    }
                    this.dimension.setBlockType(this.block.location, "minecraft:air");
                });
            }
        }
        spawnEntityOrItem(item) {
            const location = this.block.center();
            if (item.entity)
                this.dimension.spawnEntity(item.entity, location);
            if (item.item) {
                if (this.tool)
                    item.item.amount *= this.multiplier;
                this.dimension.spawnItem(item.item, location);
            }
        }
        normalizeDropItems(items) {
            let totalChance = 0;
            const itemsWithoutChance = [];
            ...
            const averageChance = itemsWithoutChance.length > 0
